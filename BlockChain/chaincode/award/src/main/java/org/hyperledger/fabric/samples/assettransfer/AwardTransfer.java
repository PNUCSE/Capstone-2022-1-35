...
public class AwardTransfer implements ContractInterface {
    ...
    public void init(final Context context) throws JsonProcessingException {
        ...
        for (Long ownerId = 1L; ownerId <= 5; ++ownerId) {
            for (int i = 0; i < 5; ++i) {
                ...
                String rank = getRandomString(List.of("rank" + ownerId + i));
                create(context, title, ownerIdStr, publisher, stub.getTxTimestamp().getEpochSecond(), rank);
            }
        }
    }
    ...
    public String create(
        ...
        ChaincodeStub stub = context.getStub();
        String id = getRandomString(Arrays.asList(title, ownerId, publisher, publishedAt.toString(), rank));
        ...
        Award award = new Award(id, title, ownerId, publisher, Instant.ofEpochSecond(publishedAt).atZone(ZoneOffset.UTC).toLocalDateTime(), stub.getTxTimestamp().atZone(ZoneOffset.UTC).toLocalDateTime(), rank);
