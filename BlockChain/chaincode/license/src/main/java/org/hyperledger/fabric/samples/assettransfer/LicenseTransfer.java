...
public final class LicenseTransfer implements ContractInterface {
    ...
    public void init(final Context context) throws JsonProcessingException {
        ...
        for (Long ownerId = 1L; ownerId <= 5; ++ownerId) {
            for (int i = 0; i < 5; ++i) {
                ...
                String description = getRandomString(List.of("state" + ownerId + i));
                String qualificationNumber = getRandomString(List.of("departmentInfo" + ownerId + i));
                create(context, title, ownerIdStr, publisher, stub.getTxTimestamp().getEpochSecond(), description, stub.getTxTimestamp().getEpochSecond(), qualificationNumber);
            }
        }
    ...
    public String create(
        ...
        ChaincodeStub stub = context.getStub();
        String id = getRandomString(Arrays.asList(title, ownerId, publisher, publishedAt.toString(), description, expireDate.toString(), qualificationNumber));
        ...
        License license = new License(id, title, ownerId, publisher, Instant.ofEpochSecond(publishedAt).atZone(ZoneOffset.UTC).toLocalDateTime(), stub.getTxTimestamp().atZone(ZoneOffset.UTC).toLocalDateTime(), description, Instant.ofEpochSecond(expireDate).atZone(ZoneOffset.UTC).toLocalDateTime(), qualificationNumber);
