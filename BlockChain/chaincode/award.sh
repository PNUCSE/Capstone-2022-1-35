...
getAll() {
    ...
    USER=$1
    peer chaincode query -C $CHANNEL_NAME -n $CC_NAME -c "{\"Args\":[\"getAll\", \"awardCC\", \"${CHANNEL_NAME}\", \"${USER}\"]}"
}
...
read() {
    ...
    peer chaincode query -C $CHANNEL_NAME -n $CC_NAME -c "{\"Args\":[\"read\", \"awardCC\", \"$CHANNEL_NAME\", \"${USER}\", \"${ASSETID}\"]}"
}
...
#     peer chaincode query -C $CHANNEL_NAME -n $CC_NAME -c "{\"Args\":[\"exists\", \"awardCC\", \"$CHANNEL_NAME\", \"${ID}\"]}"
# }

if [ $1 = "getAll" ]; then
    setVariables awardchannel readOrganizations readCC peer0 readOrg ReadOrgMSP 5051
    getAll $2
elif [ $1 = "read" ]; then
    setVariables awardchannel readOrganizations readCC peer0 readOrg ReadOrgMSP 5051
    read $2 $3
elif [ $1 = "create" ]; then
    setVariables awardchannel awardOrganizations awardCC peer0 awardOrg1 AwardOrg1MSP 7051 peer0 awardOrg2 AwardOrg2MSP 7052 peer0 awardOrg3 AwardOrg3MSP 7053
    create $2 $3 $4 $5 $6
# elif [ $1 = "exists" ]; then
#     setVariables awardchannel readOrganizations readCC peer0 readOrg ReadOrgMSP 5051
#     # setVariables awardchannel readOrganizations awardCC peer0 readOrg ReadOrgMSP 5051
#     exists $2
fi